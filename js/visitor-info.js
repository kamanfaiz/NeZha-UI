 ...
 * 在引入此模块前设置这些配置项
 */
window.VisitorInfoConfig = window.VisitorInfoConfig || {
  enabled: true, // 【访客信息】是否启用访客信息显示 (true/false)
  position: "right", // 【访客信息位置】面板显示位置 ("left" 或 "right")
  autoHideDelay: 2600 // 【访客信息自动隐藏】首次加载时自动隐藏的延迟时间 (毫秒)
};

/**
 * =================================================================
 * 内置工具函数 (Built-in Utility Functions)
 * =================================================================
 */
...
function initVisitorInfo() {
  ...
  function displayVisitorInfo(data) {
    ...
    if (window.innerWidth > 768) {
      ...
      // 设置容器位置
      const positionStyles = config.position === "left" 
      ...
      const btnPositionStyles = config.position === "left"
        ? { left: "20px", right: "auto" }
        : { right: "20px", left: "auto" };
      ...
      }, config.autoHideDelay || 2000); // 使用配置的时间
    }
  }
}
...
// 将初始化函数暴露到全局作用域
window.NeZhaVisitorInfo = {
  init: initVisitorInfo
};
