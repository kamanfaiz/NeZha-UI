...
/**
 * 插图模块配置 (使用NeZha面板标准变量名)
 * 在引入此模块前设置这些配置项
 */
// 设置默认值（如果用户未配置）
window.CustomIllustration = window.CustomIllustration || "https://cdn.jsdelivr.net/gh/kamanfaiz/NeZha-UI@main/img/Candice.png"; // 【自定义插图】右下角的插图URL
window.IllustrationStyleConfig = window.IllustrationStyleConfig || {
  width: "120px", // 【插图宽度】插图的宽度 (例如: "120px", "10vw")
  right: "-10px", // 【右边距】距离右边的距离 (例如: "-10px", "20px")
  top: "-120px", // 【上边距】距离顶部的距离 (例如: "-120px", "50px")
  zIndex: "10", // 【层级】插图的显示层级
  transitionDuration: "0.4s", // 【过渡时间】动画过渡时间 (例如: "0.4s", "0.6s")
  initialOffset: "20px" // 【初始偏移】插图出现时的初始偏移距离
};
...
function initIllustration() {
  // 检查是否禁用了看板娘动画，只有禁用时自定义插图才会生效
  if (!window.DisableAnimatedMan) {
    console.log('插图模块: 看板娘动画未禁用，自定义插图不会显示。请设置 window.DisableAnimatedMan = true');
    return;
  }
  
  if (!window.CustomIllustration || window.CustomIllustration.trim() === "") return;
  ...
  const setupIllustration = () => {
    const styleConfig = window.IllustrationStyleConfig;
    img.style.position = "absolute";
    img.style.right = styleConfig.right;
    img.style.top = styleConfig.top;
    img.style.zIndex = styleConfig.zIndex;
    img.style.width = styleConfig.width;
    img.style.transition = `opacity ${styleConfig.transitionDuration} ease-in-out, transform ${styleConfig.transitionDuration} ease-in-out`;
    img.style.opacity = "0";
    img.style.transform = `translateY(${styleConfig.initialOffset})`;
...
// 将初始化函数暴露到全局作用域
window.NeZhaIllustration = {
  init: initIllustration
};
