 ...
 * 在引入此模块前设置这些配置项
 */
window.EffectRainConfig = window.EffectRainConfig || {
  enabled: true, // 【下雨特效】是否启用背景下雨特效 (true/false)
  densityFactor: 0.0004, // 【雨滴密度】每平方像素的雨滴数。值越大，雨越密。建议范围: 0.0002 - 0.001
  maxDrops: 4000, // 【最大雨滴数】防止在超高分辨率屏幕上因雨滴过多导致性能下降
  minDrops: 200 // 【最小雨滴数】确保在小屏幕上也能看到足够的雨滴效果
};

/**
 * 初始化下雨特效功能
 */
function initEffectRain() {
  const config = window.EffectRainConfig;
  ...
  const resize = () => {
    ...
    const calculatedDrops = Math.floor(w * h * config.densityFactor);
    // 2. 将计算出的数量限制在最大和最小范围内
    const newDropCount = Math.max(
      config.minDrops,
      Math.min(calculatedDrops, config.maxDrops)
    );
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initEffectRain);
} else {
  initEffectRain();
}

// 将初始化函数暴露到全局作用域
window.NeZhaEffectRain = {
  init: initEffectRain
};
